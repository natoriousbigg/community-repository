...
function Script(TranslateToEnglish, SkipOriginalLanguage, OverWriteExistingSubtitles = false, DebugMode, NoGpu, FixAudioLanguages, SubtitleSaveDir, DisableVAD, DisablePostProcessing) {
    ...
    const postProcessSrt = (srtPath) => {
        try {
            ...
            Logger.ILog(`[whisper-sub] Post-processing: Reading file...`);
            let content;
            try {
                content = System.IO.File.ReadAllText(srtPath);
                Logger.ILog(`[whisper-sub] Post-processing: Read ${content.length} characters from file`);
            } catch (readErr) {
            ...
            Logger.ILog(`[whisper-sub] Post-processing: Parsed ${entries.length} subtitle entries`);
            ...
            // Write back to file
            Logger.ILog(`[whisper-sub] Post-processing: Writing ${processed.length} entries back to file...`);
            ...
            Logger.ELog(`[whisper-sub] Stack trace: ${err.stack || 'No stack trace available'}`);
            return false;
        }
    };
    ...
    for (let i = 0; i < audioStreams.length; i++) {
        ...
        if (keepOriginal) {
            ...
            // Verify subtitle was created successfully before post-processing
            if (System.IO.File.Exists(targetSrt)) {
                const fileInfo = new System.IO.FileInfo(targetSrt);
                if (fileInfo.Length > 0) {
                    Logger.ILog(`[whisper-sub] Subtitle file verified: ${targetSrt} (${fileInfo.Length} bytes)`);
                    if (!disablePostProcessing) {
                        const postProcessResult = postProcessSrt(targetSrt);
                        if (!postProcessResult) {
                            Logger.WLog(`[whisper-sub] Post-processing completed with warnings for ${targetSrt}`);
                        }
                    }
                } else {
                    Logger.WLog(`[whisper-sub] Subtitle file is empty: ${targetSrt}`);
                }
            } else {
                Logger.ELog(`[whisper-sub] Subtitle file not found after creation: ${targetSrt}`);
            }
        }
        ...
        if (translateToEnglish) {
            ...
            if (normalizedSourceLang === 'en') {
                ...
                if (!keepOriginal) {
                    ...
                    // Verify subtitle was created successfully before post-processing
                    if (System.IO.File.Exists(englishSrt)) {
                        const fileInfo = new System.IO.FileInfo(englishSrt);
                        if (fileInfo.Length > 0) {
                            Logger.ILog(`[whisper-sub] Subtitle file verified: ${englishSrt} (${fileInfo.Length} bytes)`);
                            if (!disablePostProcessing) {
                                const postProcessResult = postProcessSrt(englishSrt);
                                if (!postProcessResult) {
                                    Logger.WLog(`[whisper-sub] Post-processing completed with warnings for ${englishSrt}`);
                                }
                            }
                        } else {
                            Logger.WLog(`[whisper-sub] Subtitle file is empty: ${englishSrt}`);
                        }
                    } else {
                        Logger.ELog(`[whisper-sub] Subtitle file not found after creation: ${englishSrt}`);
                    }
                }
            ...
            // Verify subtitle was created successfully before post-processing
            if (System.IO.File.Exists(translatedSrt)) {
                const fileInfo = new System.IO.FileInfo(translatedSrt);
                if (fileInfo.Length > 0) {
                    Logger.ILog(`[whisper-sub] Subtitle file verified: ${translatedSrt} (${fileInfo.Length} bytes)`);
                    if (!disablePostProcessing) {
                        const postProcessResult = postProcessSrt(translatedSrt);
                        if (!postProcessResult) {
                            Logger.WLog(`[whisper-sub] Post-processing completed with warnings for ${translatedSrt}`);
                        }
                    }
                } else {
                    Logger.WLog(`[whisper-sub] Subtitle file is empty: ${translatedSrt}`);
                }
            } else {
                Logger.ELog(`[whisper-sub] Subtitle file not found after creation: ${translatedSrt}`);
            }
