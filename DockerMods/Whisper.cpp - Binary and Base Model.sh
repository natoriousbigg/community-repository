...
# Description: Installs the whisper.cpp binary (auto-detects GPU type and CPU capabilities) and downloads
#              only the base model ggml-base.bin into /app/common/whispercpp/models.
...
if [ -f "${BASE_MODEL}" ]; then
    log "Base model already present at ${BASE_MODEL}; skipping download."
else
    log "Downloading base model to ${BASE_MODEL}."
    curl -L --fail "${MODEL_URL}" -o "${BASE_MODEL}"
fi

log "whisper.cpp installation complete. Model installed under ${MODEL_DIR}."
exit 0
